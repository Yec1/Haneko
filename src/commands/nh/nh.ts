...
const command: Command = {
  data: new SlashCommandBuilder()
    ...
    .addSubcommandGroup(group =>
      group.setName("admin").setDescription("伺服器管理員設定")
        .addSubcommand(sub =>
          sub.setName("blacklist").setDescription("查看伺服器黑名單 Tag"))
        .addSubcommand(sub =>
          sub.setName("blacklist-add")
            .setDescription("新增伺服器黑名單 Tag ID")
            .addIntegerOption(o => o.setName("tag_id").setDescription("Tag ID").setRequired(true).setMinValue(1)))
        .addSubcommand(sub =>
          sub.setName("blacklist-remove")
            .setDescription("移除伺服器黑名單 Tag ID")
            .addIntegerOption(o => o.setName("tag_id").setDescription("Tag ID").setRequired(true).setMinValue(1)))
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    try {
      ...
      if (sub === "random" && !group) {
        ...
        let gallery;
        for (let i = 0; i < 10; i++) {
          gallery = await nh.getRandomGallery();
          if (nh.filterBlacklisted([gallery], guildBlacklist.map((b: any) => b.tag_id)).length > 0) break;
        }
        if (!gallery || nh.filterBlacklisted([gallery], guildBlacklist.map((b: any) => b.tag_id)).length === 0) {
      ...
      if (group === "admin") {
        ...
        if (sub === "blacklist") {
          const list = db.nhGuildGetBlacklist(guildId) as { tag_id: number; tag_name: string | null }[];
          if (!list.length) {
            await interaction.editReply({ content: "伺服器黑名單是空的。" });
            return;
          }
          const tagIds = list.map((r: any) => r.tag_id);
          const tags = await nh.getTagsByIds(tagIds);
          ...
          await interaction.editReply({ content: `🚫 **伺服器黑名單 Tag：**\n${text}` });
          return;
        }

        if (sub === "blacklist-add") {
          const tagId = interaction.options.getInteger("tag_id", true);
          db.nhGuildAddBlacklist(guildId, tagId);
          await interaction.editReply({ content: `✅ 已將 Tag ID \`${tagId}\` 加入伺服器黑名單。` });
          return;
        }

        if (sub === "blacklist-remove") {
          const tagId = interaction.options.getInteger("tag_id", true);
          db.nhGuildRemoveBlacklist(guildId, tagId);
          await interaction.editReply({ content: `✅ 已從伺服器黑名單移除 Tag ID \`${tagId}\`。` });
          return;
        }
      }
