...
export class Logger {
  ...
  success(...args: unknown[]) { this.write("success", args); }
  command(...args: unknown[]) { this.write("command", args); }
  ...
  private write(level: string, args: unknown[]) {
    ...
    if (level === "error") { console.error(head, ...args); return; }
    if (level === "warn") { console.warn(head, ...args); return; }
