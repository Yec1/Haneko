    ...
    try {
      ...
      if (sub === "random" && !group) {
        ...
        for (let i = 0; i < 10; i++) {
          ...
          if (nh.filterBlacklisted([gallery], guildBlacklist.map(b => b.tag_id)).length > 0) break;
        }
        if (!gallery || nh.filterBlacklisted([gallery], guildBlacklist.map(b => b.tag_id)).length === 0) {
