	...
	async execute(
		...
		for (let attempt = 0; attempt <= maxRetries; attempt++) {
			try {
				...
				if (attempt > 0) {
					this.logger.success(
						`${commandName} succeeded after ${attempt} retries`
					);
				}
				return result;
			} catch (error) {
			...
			new Promise((_, reject) =>
				setTimeout(
					() =>
						reject(
							new Error(
								`Command execution exceeded ${timeoutMs}ms timeout`
							)
						),
					timeoutMs
				)
...
export class EnhancedErrorHandler {
	...
	registerRecoveryStrategy(
		errorType: string,
		handler: (error: any) => Promise<boolean>
	): void {
