...
async function openBook(tr, interaction, book) {
  ...
  const { cmdMenu, tagMenu, bookBack, bookPage, bookNext } = getBookComponents(
    tr,
    interaction.user.id,
    bookDB
  );
...
async function openBookShelf(tr, interaction, res, type, name) {
  const index = 1;
  const totalPages = res?.pagination.totalPages || 1
  const currentPage = res?.pagination.currentPage || 1

  await db.set(`${interaction.user.id}.shelf`, {
    name: type == "search" || "searchwithtag" ? name : null,
    type: type,
    currentIndex: index,
...
function getShelfComponents(tr, id, index, res) {
  ...
  return { backD, back, page, next, nextD, check };
}
